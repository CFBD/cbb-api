...
const placeholderFor = (
  ...
  return fail(
    context,
    `required parameter ${parameter.name} has unsupported type ${
      schema.type ?? 'unknown'
    }`,
  );
};
...
const getArgument = (
  ...
  if (!parameter.name) {
    return fail(context, 'parameter is missing a name');
  }
  ...
  if (!parameter.schema) {
    return fail(context, `parameter ${parameter.name} has no schema`);
  }
...
const formatTypescriptGroup = (
  location: ParameterLocation,
  argumentsForLocation: SnippetArgument[],
): string[] => {
  if (argumentsForLocation.length === 0) {
    return [];
  }

  return [
    `  ${location}: {`,
    ...argumentsForLocation.map(
      (argument) =>
        `    ${argument.name}: ${formatValue(argument.value)},${
          argument.replace ? ` // Replace with a valid ${argument.name}` : ''
        }`,
    ),
    '  },',
  ];
...
const formatTypescript = (record: SnippetRecord): string => {
  const pathArguments = record.arguments.filter(
    ({ location }) => location === 'path',
  );
  const queryArguments = record.arguments.filter(
    ({ location }) => location === 'query',
  );
  const options = [
    ...formatTypescriptGroup('path', pathArguments),
    ...formatTypescriptGroup('query', queryArguments),
  ];
  ...
  return [
    `import { client, ${record.typescriptOperation} } from 'cbbd';`,
    '',
...
const formatPython = (record: SnippetRecord): string => {
  ...
  return [
    ...
    `    ${record.pythonApiVariable} = cbbd.${record.pythonApiClass}(api_client)`,
    ...call,
    '    print(response)',
  ].join('\n');
};
    ...
    for (const [method, value] of Object.entries(pathItem)) {
      ...
      for (const argument of args) {
        if (!argument.pythonName || pythonNames.has(argument.pythonName)) {
          fail(
            context,
            `parameter ${argument.name} has an invalid Python name`,
          );
        }

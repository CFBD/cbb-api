...
describe('per-user concurrency limit middleware', () => {
  ...
  test('skips unmatched paths, methods, and unauthenticated requests', () => {
    ...
    const requests = [
      ...
      createRequest(1, '/lineups/team', 'POST'),
      getMockReq({
        method: 'GET',
        path: '/lineups/team',
      }) as unknown as Request,
  ...
  test('uses the canonical matched route for path comparison', () => {
    ...
    const first = createResponse();
    const second = createResponse();
    const request = createRequest(1, '/LiNeUpS/TeAm/');
    request.route = { path: '/lineups/team' };
  ...
  test('limits concurrent requests to the team lineups endpoint', () => {
    const first = createResponse();
    const second = createResponse();
    const third = createResponse();
    const firstNext = jest.fn();
    ...
    middlewares.concurrencyLimit(createRequest(1), first.res, firstNext);
    middlewares.concurrencyLimit(createRequest(1), second.res, secondNext);
    middlewares.concurrencyLimit(createRequest(1), third.res, thirdNext);

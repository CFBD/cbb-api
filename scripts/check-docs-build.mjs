...
const siteOrigin = 'https://api.collegebasketballdata.com';
...
const hasProductionServer = openapi.servers?.some(
  (server) => server.url === 'https://api.collegebasketballdata.com/',
);
if (!hasProductionServer) {
  fail('OpenAPI servers must include https://api.collegebasketballdata.com/');
}
...
if (
  !gamesApi.includes(
    '<title>Games - College Basketball Data API | CBBD</title>',
  )
) {
...
const publicAssets = [
  'favicon.ico',
  'brand/cbbd-watermark.png',
  'brand/cbbd-watermark-dark.png',
];
...
for (const route of authoredRoutes) {
  ...
  const assignments = markdown.matchAll(/CBBD_API_KEY\s*=\s*['"]([^'"]+)['"]/g);
